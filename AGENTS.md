...
- `src/space/bus/` - events and calls between apps (docs/events.md): `types.ts` (data model, limits), `spec.ts` (the `events:` and `provides:` manifest sections; `consumes` with `task:` becomes a trigger), `store.ts` (bun:sqlite: deliveries, calls), `bus.ts` (engine: one delivery per matching subscription, http worker with retries, stream push and ack, calls forwarded to the provider), `api.ts` (HTTP routes and the SSE stream).
