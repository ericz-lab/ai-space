...
- `src/space/defaults.ts` - the apps a space comes with (`SPACE_DEFAULT_APPS`, cloned and installed by `init`) and the per-machine `SPACE_APP_URL_<NAME>` override of a manifest's `url`.
