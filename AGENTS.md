...
- `src/space/model/` - model calls for apps and agent tasks: `types.ts` (data model), `spec.ts` (request validation), `runner.ts` (local `claude`, ssh, or API backend), `store.ts` (bun:sqlite ledger and sums), `service.ts` (concurrency cap, recording), `api.ts` (HTTP routes), `tasks.ts` (scheduler hook for agent runs), `testing.ts` (the CLI stand-in tests use). Design notes in `docs/model.md`.
...
- `docs/` - project documentation and diagrams. `docs/architecture.svg` is the architecture figure used in the README. Service designs: `scheduler.md`, `storage.md`, `backup.md` (snapshots, retention, verify, restore), `notify.md` (chat notifications, design only), `model.md` (model calls and the usage ledger), `panel.md` (web UI, chat, widgets), `peers.md` (one panel over several machines), `i18n.md` (languages of the panel and the manifest's `i18n:` section), `app-spec.md` (the app contract), `install.md` (first install on a server), and `install-by-agent.md` (the same install done by a coding agent: order, stops, checklist).
