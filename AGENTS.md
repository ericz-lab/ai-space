...
- `src/space/services/` - service supervision (docs/supervision.md), under `SPACE_SUPERVISOR=space`: `unit.ts` (the `space-<app>.service` unit and its environment file, pure), `systemd.ts` (`systemctl` with an injectable runner), `supervisor.ts` (reconciles each app's unit with its manifest on every sync, removal, conflicts with the operator's units), `api.ts` (`GET`/`POST /api/apps/:app/service`).
