...
/**
 * Scheduler hook: an agent task runs on its runtime adapter from the
 * scheduler (see scheduler/targets.ts), so its usage reaches the ledger from
 * the run result instead of through `POST /api/model/run`. The row is tagged
 * with the task name and marked `origin: task`; a run that produced no usage
 * (a timeout, an older CLI) is still recorded as a call so the count is
 * honest, with no token figures.
 */
