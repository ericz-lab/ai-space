...
export type StorageOptions = {
  ...
  apiUrl?: string;
  /** What this space calls itself (`SPACE_NAME`), handed to apps as `SPACE_NAME`; absent = not written. */
  spaceName?: string;
  log?: (msg: string) => void;
};
...
export class StorageService {
  ...
  async envFor(app: string): Promise<Record<string, string>> {
    assertName(app, "app");
    return envVars(app, this.appDataDir(app), await this.list(app), await this.blobStore(app), this.s3, await this.tokenFor(app), this.apiUrl, this.spaceName);
  }
  ...
  private async writeEnv(app: string, databases: ProvisionedDatabase[], blobs: ProvisionedBlobStore | undefined): Promise<void> {
    const vars = envVars(app, this.appDataDir(app), databases, blobs, this.s3, await this.tokenFor(app), this.apiUrl, this.spaceName);
    const lines = [
...
export function envVars(
  ...
  const vars: Record<string, string> = {
    SPACE_APP: app,
    SPACE_APP_DATA_DIR: dataDir,
    ...(token ? { SPACE_APP_TOKEN: token } : {}),
    ...(apiUrl ? { SPACE_API_URL: apiUrl } : {}),
